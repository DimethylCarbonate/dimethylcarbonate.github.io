...
同时，由于字符串本质上是一个字符数组，因此形式参数和实际参数都可以写成指向字符数组中第一位的指针。

### 返回指针的函数

函数的返回值可以是一个指针，只要在函数名前加上`*`即可。

## 引用

### 引用的定义及应用

引用是给变量（左值）起一个别名，使得一块内存空间可以通过几个变量名来访问。引用实质上是一种隐式指针，但每次使用时无需加上`*`。定义时，也可以加上`const`限定。
